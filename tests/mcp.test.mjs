...
test('MCP negotiates version, advertises six tools, and serves a typed decision', async t => {
  ...
  c.send(2, 'tools/list'); assert.deepEqual((await c.wait(2)).result.tools.map(x => x.name), ['jev_decide', 'jev_status', 'jev_feedback', 'jev_route', 'jev_filter', 'jev_observe', 'jev_record']);
