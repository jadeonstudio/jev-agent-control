...
async function layaMain(argv, env) {
  const sub = argv[0];
  if (sub === 'register') {
    const flags = parseFlags(argv.slice(1), ['home', 'checkpoint', 'model', 'device', 'precision'], []);
    if (!flags.checkpoint) fail('LAYA_CHECKPOINT_PATH_REQUIRED');
    output(registerCheckpoint(layaHome(flags, env), { checkpointDir: flags.checkpoint, model: flags.model, device: flags.device, precision: flags.precision })); return;
  }
  if (sub === 'holdout') {
    const action = argv[1];
    if (action === 'freeze') {
