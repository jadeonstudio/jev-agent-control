...
## v0.3 training capture is a separate opt-in data boundary

Operational telemetry still stores no raw state, questions or answer values. Explicitly enabled Training Capture may persist bounded sanitized content to private files OUTSIDE Git. It is OFF by default, is not encrypted storage, and performs no upload/training. Every export row is screened; regex detection is not complete DLP. Capture permission must cover the complete inference interval. Model comparisons need explicit remote consent. MCP outcomes and the `training outcome` CLI's stdin JSON are always weak host-review evidence, never runner/human attestations, regardless of what `source`/label values the caller supplies. Laya starts only a locally configured Python/runtime/checkpoint with allowlisted environment, no inherited API/HF secrets, no shell, no network download, and no automatic CPU fallback acceptance. Same-OS-user tampering is outside the threat boundary. See [docs/TRAINING_DATA.md](docs/TRAINING_DATA.md).

Strong (`objective`/`human`) labels can only be created through: an in-process library caller that owns its own runner (`store.outcome()`, e.g. a directly integrated executor); `jev-control runner verify`, which runs only a command an operator pre-registered from an interactive terminal with `jev-control runner allow` (the agent selects a registered name, never the argv, labels, or pass/fail mapping); and `jev-control training correct`, an interactive human-correction prompt. `runner allow`/`remove` and `training correct` all require both stdin and stdout to be a TTY. **A TTY check is a same-OS-user local-authorship signal, not proof of a human operator** — a PTY-attached coding agent also has a TTY, and this is not a defense against that same-OS-user threat, consistent with the "Trust boundaries" section above. `runner verify` never lets its command's stdout/stderr, or a caller-supplied exit code/label, reach storage: jev runs the registered argv itself with `stdio: 'ignore'` and derives the recorded label only from the process it started. A timeout, spawn failure or signal death records a failed command check but no label, and the command runs without `TYPESAFE_API_KEY` in its environment. Changing the dataset minimum-label gate also requires a TTY.
